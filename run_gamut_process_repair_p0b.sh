...
OUT_DIR="${GAMUT_P0B_OUT:-$PROJECT_SCR/results/gamut_process_repair_p0b_${SLURM_JOB_ID:-manual}}"
...
for model_dir in \
  ...
  [ -d "$model_dir" ] || { echo "FATAL: cached model missing at $model_dir" >&2; exit 3; }
done
...
"$PY" -X utf8 -m unittest -v test_gamut_process_repair_p0.py test_gamut_process_repair_p0b.py
"$PY" -X utf8 gamut_process_repair_p0b.py \
  --dataset "$DATA" --split test_text_only \
  --generator-model Qwen/Qwen2.5-7B-Instruct \
  --judge-model Qwen/Qwen2.5-14B-Instruct \
  --skip 48 --n 192 --generation-batch-size 4 --judge-batch-size 8 \
  --out-dir "$OUT_DIR"
sha256sum "$OUT_DIR/gamut_process_repair_p0b_report.json" \
  "$OUT_DIR/gamut_process_repair_p0b_baselines.jsonl" \
  "$OUT_DIR/gamut_process_repair_p0b_candidates.jsonl"
